...
class StrategyGridTradingBreakOut(rtstr.RealTimeStrategy):
    ...
    def __init__(self, params=None):
        ...
        self.nb_grid = 100              # CEDE to be added in param
        self.percent_per_grid = 0.5     # CEDE to be added in param

        self.grid_up = GridPosition(self.lst_symbols, 0, self.grid_high, self.nb_grid, self.percent_per_grid,
                                    self.zero_print, self.loggers)
        self.grid_down = GridPosition(self.lst_symbols, self.grid_low, 0, self.nb_grid, self.percent_per_grid,
                                      self.zero_print, self.loggers)
    ...
    def get_info(self):
        return "StrategyGridTradingBreakOut"
    ...
    def set_normalized_grid_price(self, lst_symbol_plc_endstp):
        for price_plc in lst_symbol_plc_endstp:
            self.grid_up.normalize_grid_price(price_plc['symbol'], price_plc['pricePlace'], price_plc['priceEndStep'])
            self.grid_down.normalize_grid_price(price_plc['symbol'], price_plc['pricePlace'], price_plc['priceEndStep'])
...
class GridPosition():
    def __init__(self, lst_symbols, grid_high, grid_low, nb_grid, percent_per_grid, debug_mode=True, loggers=[]):
        ...
        if self.grid_high == 0:
            self.grid_type = "grid_up"
        elif self.grid_low == 0:
            self.grid_type = "grid_down"
        ...
        self.df_nb_open_positions = pd.DataFrame(
            columns=['symbol', 'size', 'leverage', 'side', 'positions_size', 'nb_open_positions'])
        ...
        if self.grid_type == "grid_up":
            if self.percent_per_grid != 0:
                self.steps = self.grid_low * self.percent_per_grid / 100
                self.grid_high = self.grid_low + self.nb_grid * self.percent_per_grid * self.grid_low / 100
                self.lst_grid_values = np.linspace(self.grid_high, self.grid_low, self.nb_grid + 1, endpoint=True).tolist()
                self.nb_grid_high = len(self.lst_grid_values)
        elif self.grid_type == "grid_down":
            if self.percent_per_grid != 0:
                self.nb_grid = 500
                self.steps = self.grid_high * self.percent_per_grid / 100
                self.grid_low = self.grid_high - self.nb_grid * self.percent_per_grid * self.grid_high / 100
                self.lst_grid_values = np.linspace(self.grid_high, self.grid_low, self.nb_grid + 1, endpoint=True).tolist()
                self.lst_grid_values = [x for x in self.lst_grid_values if x > 0]
                self.lst_grid_values.reverse()
                self.nb_grid_low = len(self.lst_grid_values)
        ...
        for grid_id in lst_grid_id:
            ...
            if grid_id in df_current_state["gridId"].tolist():
                if side == df_current_state.loc[df_current_state["gridId"] == grid_id, 'side'].values[0]:
                    ...
                    df_current_state.loc[df_current_state["gridId"] == grid_id, 'orderId'].values[0]
                else:
    ...
    def get_order_list(self, symbol, size, df_current_order):
        ...
        lst_gridId_existing_orders = [df_grid.loc[df_grid['position'] == price, 'grid_id'].values[0] for price in
                                      lst_price_existing_orders]
    ...
    def update_nb_open_positions(self, symbol, df_open_positions, buying_size):
        ...
        if len(df_open_positions) > 0:
            filtered_df = df_open_positions[
                (df_open_positions['symbol'] == symbol) & (df_open_positions['holdSide'] == "long")]
            self.df_nb_open_positions.loc[self.df_nb_open_positions['symbol'] == symbol, 'nb_previous_open_positions'] = \
            self.df_nb_open_positions.loc[self.df_nb_open_positions['symbol'] == symbol, 'nb_open_positions'].values[0]
            if len(filtered_df) > 0:
                ...
                self.df_nb_open_positions.loc[
                    self.df_nb_open_positions['symbol'] == symbol, 'positions_size'] = sum_available_position
                ...
                self.df_nb_open_positions.loc[
                    self.df_nb_open_positions['symbol'] == symbol, 'nb_open_positions'] = nb_open_positions
                previous_nb_open_positions = self.df_nb_open_positions.loc[
                    self.df_nb_open_positions['symbol'] == symbol, 'nb_previous_open_positions'].values[0]
                self.df_nb_open_positions['previous_nb_open_positions'] = previous_nb_open_positions
                self.diff_open_position = self.nb_open_positions - self.control_multi_position['nb_previous_open_long']
                self.diff_close_position = self.nb_close_positions - self.control_multi_position[
                    'nb_previous_close_long']
                self.df_nb_open_positions.loc[
                    self.df_nb_open_positions['symbol'] == symbol, 'diff_open_position'] = self.diff_open_position
                self.df_nb_open_positions.loc[
                    self.df_nb_open_positions['symbol'] == symbol, 'diff_close_position'] = self.diff_close_position
            ...
            self.diff_open_position = self.nb_open_positions - self.control_multi_position['nb_previous_open_long']
            self.diff_close_position = self.nb_close_positions - self.control_multi_position['nb_previous_close_long']
            self.df_nb_open_positions.loc[
                self.df_nb_open_positions['symbol'] == symbol, 'diff_open_position'] = self.diff_open_position
            self.df_nb_open_positions.loc[
                self.df_nb_open_positions['symbol'] == symbol, 'diff_close_position'] = self.diff_close_position
    ...
    def filter_lst_close_execute_order(self, symbol, lst_order_to_execute):
        ...
        nb_close_selected_to_be_open = self.control_multi_position[
                                           'nb_open_long_triggered'] + self.remaining_close_to_be_open
        ...
        # nb_open_selected_to_be_open = self.control_multi_position['nb_close_long_triggered']
        ...
        if (nb_close_selected_to_be_open > 0) \
                and (nb_close_in_order_list < self.control_multi_position['nb_open_long_triggered']):
            self.remaining_close_to_be_open += self.control_multi_position[
                                                   'nb_open_long_triggered'] - nb_close_in_order_list
        if nb_close_in_order_list > self.control_multi_position['nb_open_long_triggered']:
            self.remaining_close_to_be_open = self.remaining_close_to_be_open - (
                        nb_close_in_order_list - self.control_multi_position['nb_open_long_triggered'])
        if self.remaining_close_to_be_open < 0:
            self.remaining_close_to_be_open = 0

        count_close_order_in_lst = sum(
            1 for d in filtered_orders_to_execute if d.get("type") in ["CLOSE_LONG_ORDER", "CLOSE_SHORT_ORDER"])
        if count_close_order_in_lst > 0 \
                and (count_lst_close_engaged + count_close_order_in_lst) > self.df_nb_open_positions.loc[
            self.df_nb_open_positions['symbol'] == symbol, 'nb_open_positions'].values[0]:
            nb_close_to_drop_from_list = \
            self.df_nb_open_positions.loc[self.df_nb_open_positions['symbol'] == symbol, 'nb_open_positions'].values[
                0] - (count_lst_close_engaged + count_close_order_in_lst)
            ...
            self.log("nb_open_positions: " + str(self.df_nb_open_positions.loc[self.df_nb_open_positions[
                                                                                   'symbol'] == symbol, 'nb_open_positions'].values[
                                                     0]))
    ...
    def init_control_multi_position(self):
        ...
        multi_position['nb_open_long_triggered'] = 0
    ...
    def update_open_orders(self, symbol, df_current_states):
        previous_nb_open_limit_order = \
        self.df_nb_open_positions.loc[self.df_nb_open_positions['symbol'] == symbol, 'nb_open_limit_order'].values[0]
        self.df_nb_open_positions.loc[self.df_nb_open_positions[
                                          'symbol'] == symbol, 'previous_nb_open_limit_order'] = previous_nb_open_limit_order
        nb_open_limit_order = len(
            df_current_states[(df_current_states['symbol'] == symbol) & (df_current_states['side'] == "open_long")])
        self.df_nb_open_positions.loc[
            self.df_nb_open_positions['symbol'] == symbol, 'nb_open_limit_order'] = nb_open_limit_order
        ...
        self.nb_close_positions = len(
            df_current_states[(df_current_states['symbol'] == symbol) & (df_current_states['side'] == "close_long")])
    ...
    def update_control_multi_position(self, symbol, df_current_state, df_open_positions):
        self.control_multi_position['nb_current_limit_close'] = len(
            df_current_state[(df_current_state['side'] == 'close_long')])
        self.control_multi_position['nb_current_limit_open'] = len(
            df_current_state[(df_current_state['side'] == 'open_long')])
        ...
        lst_open_long_triggered = [element for element in self.control_multi_position['lst_previous_open_long'] if
                                   element not in self.control_multi_position['lst_new_open_long']]
        self.control_multi_position['lst_open_long_triggered'] = lst_open_long_triggered
        self.control_multi_position['nb_open_long_triggered'] = len(lst_open_long_triggered)

        lst_close_long_triggered = [element for element in self.control_multi_position['lst_previous_close_long'] if
                                    element not in self.control_multi_position['lst_new_close_long']]
        ...
        self.control_multi_position['lst_open_long'] = utils.drop_smallest_items(
            self.control_multi_position['lst_open_long'],
            self.control_multi_position['nb_close_long_triggered'])

        self.control_multi_position['lst_open_long'] = self.control_multi_position[
                                                           'lst_open_long'] + lst_open_long_triggered
        ...
        del condition_pending
        del condition_open_long
        del condition_close_long
        del df_filtered_previous_open_long
        del df_filtered_previous_close_long
        del lst_previous_close_long
        del condition_symbol
        del df_filtered_new_open_long
        del lst_new_open_long
        del df_filtered_new_close_long
        del lst_new_close_long
        del lst_open_long_triggered
        del lst_close_long_triggered
        del df_current_state
        del df_open_positions
    ...
    def filter_close_limit_order(self, open_grid_id):
        if len(self.control_multi_position['lst_open_long_triggered']) > 0:
            top_open_triggered = min(self.control_multi_position['lst_open_long_triggered'])
    ...
    def get_grid_info(self, symbol):
        ...
        self.dct_status_info['nb_limit_close'] = len(df[(df['side'] == 'close_long') & (df['status'] == 'engaged')])

        # Count of rows with side == 'open_long' and status == 'engaged'
        self.dct_status_info['nb_limit_open'] = len(df[(df['side'] == 'open_long') & (df['status'] == 'engaged')])
    ...
    def dct_status_info_to_txt(self, symbol):
        ...
        if ((self.dct_status_info['grid_position'][0] >= self.nb_grid)
            ...
            self.msg += "**pos range: " + str(self.dct_status_info['grid_position'][0]) + " / " + str(
                self.dct_status_info['grid_position'][1]) + "**\n"
        ...
        if len(self.previous_grid_position) == 2:
            self.msg += "# TREND:" + "\n"
            if self.previous_grid_position[0] > self.dct_status_info['grid_position'][0] and \
                    self.previous_grid_position[1] > self.dct_status_info['grid_position'][1]:
                self.msg += "DOWN" + "\n"
            elif self.previous_grid_position[0] < self.dct_status_info['grid_position'][0] and \
                    self.previous_grid_position[1] < self.dct_status_info['grid_position'][1]:
                self.msg += "UP" + "\n"
            elif self.previous_grid_position[0] == self.dct_status_info['grid_position'][0] and \
                    self.previous_grid_position[1] == self.dct_status_info['grid_position'][1]:
        ...
        if (self.max_grid_open_order == -1) \
            ...
            if self.nb_open_missing > 0:
                self.msg += "open range: " + str(self.min_grid_open_order) + " / " + str(
                    self.max_grid_open_order) + " gaps: " + str(self.nb_open_missing) + "\n"
            else:
                self.msg += "open range: " + str(self.min_grid_open_order) + " / " + str(
                    self.max_grid_open_order) + "\n"
        ...
        if (self.max_grid_close_order == -1) \
            ...
            if self.nb_close_missing > 0:
                self.msg += "close range: " + str(self.min_grid_close_order) + " / " + str(
                    self.max_grid_close_order) + " gaps: " + str(self.nb_close_missing) + "\n"
            else:
                self.msg += "close range: " + str(self.min_grid_close_order) + " / " + str(
                    self.max_grid_close_order) + "\n"
