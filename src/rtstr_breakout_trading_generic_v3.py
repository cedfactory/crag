...
class StrategyBreakoutTradingGenericV3(rtstr.RealTimeStrategy):
    ...
    def __init__(self, params=None):
        ...
        if params:
            ...
            self.symbol = params.get("strategy_symbol", None)
    ...
    def get_info(self):
        return "StrategyBreakoutTradingGenericV3"
    ...
    def set_broker_current_state(self, current_state):
        ...
        if self.symbol in df_price['symbols'].tolist():
            self.grid.set_current_price(df_price.loc[df_price['symbols'] == self.symbol, 'values'].values[0])
            self.grid.set_triggers_executed(df_open_triggers)
            self.grid.cross_check_with_current_state(self.symbol)
            self.grid.update_grid_side(self.symbol)
    ...
    def print_debug_grid(self):
        ...
        if self.backup_grid is None:
            ...
            for trend in lst_trend:
                ...
                df_to_save = self.grid.grid[self.symbol][trend].drop(columns=exclude_columns).copy()
                df_backup_to_save = self.backup_grid[self.symbol][trend].drop(columns=exclude_columns).copy()
    ...
    def set_breakout_grid_buying_size(self, dol_per_grid):
        self.grid.set_breakout_grid_buying_size(dol_per_grid)
    ...
    def set_df_buying_size(self, df_symbol_size, cash):
        ...
        if (self.get_grid_buying_min_size(self.symbol) <= (size / self.grid.get_max_price_in_grid())) \
            ...
            msg += "**min size: " + str(self.get_grid_buying_min_size(self.symbol)) + " - $" + str(self.get_grid_buying_min_size(self.symbol) * (self.grid_high + self.grid_low )/2) + "**\n"
...
class GridPosition():
    def __init__(self, side, symbol, grid_high, grid_low, nb_grid, percent_per_grid, nb_position_limits, strategy_id, debug_mode=True, loggers=[]):
        self.grid_side = side
        self.side_mapping = {
            "up": "trigger_long",
            "down": "trigger_short"
        ...
        for trend in self.lst_trend:
            ...
            self.max = max(self.grid[self.symbol][trend]["position"].to_list())
            self.max_position = max(self.max, self.max_position)

            self.grid[self.symbol][trend]["grid_id"] = self.grid[self.symbol][trend].index
            for col in ["orderId_trigger_long", "orderId_trailer_long",
                        "orderId_trigger_short", "orderId_trailer_short",
                        "status_trigger_long", "status_trailer_long",
                        "status_trigger_short", "status_trailer_short",
                        "status_long", "status_short"]:
                self.grid[self.symbol][trend][col] = "empty"
            for col in ["side"]:
                self.grid[self.symbol][trend][col] = ""
    ...
    def set_breakout_grid_buying_size(self, dol_per_grid):
        for trend in self.lst_trend:
            df_grid = self.grid[self.symbol][trend]
    ...
    def set_triggers_executed(self, df_open_triggers):
        df_grid_tend = self.grid[self.symbol]
        condition_strategy_id = df_open_triggers['strategyId'] == self.strategy_id
        condition_trigger_symbol = df_open_triggers['symbol'] == self.symbol
        ...
        for trend in self.lst_trend:
            ...
            if nb_new_SLTP == 0 \
                ...
                if len(self.lst_cancel_order_sltp) > 0:
                    list_of_dicts = self.transform_order_ids_to_dict(self.symbol, self.lst_cancel_order_sltp)
    ...
    def transform_order_ids_to_dict(self, symbol, order_ids):
        ...
        return [{'symbol': base_symbol, 'orderId': order_id, 'type': 'CANCEL_SLTP'} for order_id in order_ids]
