...
class TestRTDP:
    ...
    text_no_reason = "no reason"

    def test_rtdp_tv_next(self, mocker):
        
        # context
        my_rtdp_tv = rtdp_tv.RTDPTradingView()
        mocker.patch(self.src_utils_fdp_request, side_effect=[self.response_reco, self.response_symbol])
    ...
    def test_rtdp_tv_next_response_recommendations_ko(self, mocker):
        response_recommendations = {'elapsed_time': '0:00:02.537734', 'status': 'ko', 'reason': self.text_no_reason, 'result': {}}

        # context
        my_rtdp_tv = rtdp_tv.RTDPTradingView()
        mocker.patch(self.src_utils_fdp_request, side_effect=[response_recommendations, self.response_symbol])
    ...
    def test_rtdp_tv_next_response_symbol_ko(self, mocker):
        response_symbol = {'elapsed_time': '0:00:02.537734', 'status': 'ko', 'reason': self.text_no_reason, 'result': {'status': 'ko', 'reason': self.text_no_reason, 'symbols': {}}}

        # context
        my_rtdp_tv = rtdp_tv.RTDPTradingView()
        mocker.patch(self.src_utils_fdp_request, side_effect=[self.response_reco, response_symbol])
